...
if ((await GM_getValue("capacityCounter", true)) === true)
  await GM_setValue("capacityCounter", 0);
// access while down functionality
if (document.getElementsByClassName("text-3xl font-medium").length > 0) {
  await GM_setValue(
    "capacityCounter",
    (await GM_getValue("capacityCounter")) + 1
  );
  if ((await GM_getValue("capacityCounter")) > 8) {
    await GM_setValue("capacityCounter", 0);
    changeFavicon("blue");
    alert("ChatGPT is down. Accessing anyway...");
